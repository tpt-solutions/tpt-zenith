...
fn perr(a: &StateVector, e: &[f64; 3]) -> f64 {
    ((a.position_km[0] - e[0]).powi(2)
        + (a.position_km[1] - e[1]).powi(2)
        + (a.position_km[2] - e[2]).powi(2))
    .sqrt()
}
...
fn main() {
    ...
    let detail: std::collections::HashSet<&str> = std::collections::HashSet::new();
    ...
    for ((l1, l2), rows) in cases.iter().zip(blocks.iter()) {
        ...
        let p = match Propagator::from_tle(&t) {
            Ok(p) => p,
            Err(e) => {
                res.insert(sat, (f64::INFINITY, format!("init_err {e}")));
                continue;
            }
        };
