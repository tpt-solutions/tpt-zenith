...
const Q0: f64 = 120.0;
const S0: f64 = 78.0;
const QOMS2T: f64 = ((Q0 - S0) / XKMPER) * ((Q0 - S0) / XKMPER) * ((Q0 - S0) / XKMPER) * ((Q0 - S0) / XKMPER);
...
fn init(t: &Tle) -> Sat {
    ...
    let c2 = coef1 * n * (a * (1.0 + 1.5 * etasq + eeta * (4.0 + etasq))
        + 0.75 * CK2 * tsi / psisq * x3thm1 * (8.0 + 3.0 * etasq * (8.0 + etasq)));
    let c1 = bstar * c2;
    let c4 = 2.0 * n * coef1 * a * betao2
    ...
    let mdot = n + 0.5 * temp1 * betao * x3thm1 + 0.0625 * temp2 * betao * (13.0 - 78.0 * theta2 + 137.0 * theta4);
    ...
    if ecco > 0.0085 && ecco < 0.0088 {
        eprintln!("XCINIT n={} mdot={} temp1={} pinvsq={} a={} betao2={}", n, mdot, temp1, pinvsq, a, betao2);
    }

    Sat {
        inclo, nodeo, ecco, argpo, mo, no: n, bstar,
        con41, x1mth2, x7thm1, xlcof, aycof,
        c1, c4, mdot, omgdot, xnodot, xnodcf, t2cof,
        c5, omgcof, xmcof, delmo, sinmo, d2, d3, d4, t3cof, t4cof, t5cof, eta,
    }
}
...
fn propagate(s: &Sat, tsince: f64) -> [f64; 3] {
    ...
    if s.ecco > 0.0085 && s.ecco < 0.0088 {
        eprintln!("XC t={} a_val={} xl={} xmp={} omega={} xnode={} templ={} tempa={} tempe={} e={}", tsince, a_val, xl, xmp, omega, xnode, templ, tempa, tempe, e);
    }
    let beta2 = 1.0 - e * e;
...
fn main() {
    ...
    for ((l1, l2), rows) in cases.iter().zip(blocks.iter()) {
        let sat = &l1[2..7];
        let t = match Tle::parse_lines(None, l1, l2) { Ok(t)=>t, Err(_)=>continue };
        let s = init(&t);
        ...
        for r in rows {
            ...
            if e > worst { worst = e; }
            if sat == "88888" {
                println!("XC88888 t={:.1} ref={:?} got={:?} err={:.3e}", r.tsince, r.pos, got, e);
            }
        }
...
fn parse_cases(text: &str) -> Vec<(String, String)> {
    ...
    for line in text.lines() {
        ...
        if line.starts_with("1 ") { line1 = Some(line.to_string()); }
        else if line.starts_with("2 ") { if let Some(l1) = line1.take() { cases.push((l1, line.to_string())); } }
    }
    cases
}
...
fn parse_expected(text: &str) -> Vec<Vec<Row>> {
    ...
    for line in text.lines() {
        ...
        if toks.len() == 2 && toks[1] == "xx" { blocks.push(Vec::new()); continue; }
        if toks.len() >= 7 {
            if let (Ok(tsince), Ok(x), Ok(y), Ok(z)) = (toks[0].parse::<f64>(), toks[1].parse::<f64>(), toks[2].parse::<f64>(), toks[3].parse::<f64>()) {
                let r = (x*x+y*y+z*z).sqrt();
                if tsince.abs() <= 100_000.0 && r <= 100_000.0 {
                    if let Some(b) = blocks.last_mut() { b.push(Row { tsince, pos: [x,y,z] }); }
                }
            }
        }
    }
